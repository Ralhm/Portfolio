...
public class MapController : MonoBehaviour
{
    ...
    public Dictionary<Door, Room> DoorRoomMap = new Dictionary<Door, Room>();

    public Dictionary<Vector2Int, Door> DoorMap = new Dictionary<Vector2Int, Door>();
    ...
    public void RegisterRoom(Room room)
    {
        ...
        //Create a map so that we can quickly and easily find what room contains what door
        for (int i = 0; i < room.Doors.Count; i++) { 
            DoorRoomMap.Add(room.Doors[i], room);
            
        
        }

        for (int i = 0; i < room.roomInfo.Tiles.Count; i++) {
            RoomMap.Add(room.roomInfo.Tiles[i], room);
        }

        //RoomMap.Add(vec, room);
    ...
    //Then cycle through all the rooms, which then cycle through all their doors, to find their matching door.
